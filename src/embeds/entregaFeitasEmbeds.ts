...
export const enviarNotificacaoEntregaFeita = async (cliente: Client, grupo: any) => {
    const notificacaoConfig = config.entregas_feitas.notificacao;
    const subServidorConfig = config.entregas_feitas.usar_sub_servidor ? config.entregas_feitas.sub_servidores.find((s: any) => s.nome === grupo.subservidor) : null;
    const canalId = subServidorConfig ? subServidorConfig.canal : config.entregas_feitas.canal_de_entregas_geral;
